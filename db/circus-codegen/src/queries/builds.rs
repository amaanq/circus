...
pub fn start() -> StartStmt {
    StartStmt(
        "WITH candidate AS ( SELECT id FROM builds WHERE id = $1 AND status = 'pending' FOR UPDATE SKIP LOCKED ) UPDATE builds SET status = 'running', started_at = NOW() FROM candidate WHERE builds.id = candidate.id RETURNING builds.*",
        None,
    )
}
impl StartStmt {
...
pub fn requeue() -> RequeueStmt {
    RequeueStmt(
        "WITH bumped AS ( UPDATE builds SET status = 'pending', started_at = NULL, completed_at = NULL WHERE id = $1 AND status = 'running' RETURNING * ), cleared AS ( DELETE FROM build_steps WHERE build_id = $1 AND EXISTS (SELECT 1 FROM bumped) ) SELECT * FROM bumped",
        None,
    )
}
impl RequeueStmt {
...
pub fn retry() -> RetryStmt {
    RetryStmt(
        "UPDATE builds SET status = 'pending', started_at = NULL, retry_count = retry_count + 1, completed_at = NULL WHERE id = $1 RETURNING *",
        None,
    )
}
impl RetryStmt {
...
pub fn list_pending_in_scheduler_order() -> ListPendingInSchedulerOrderStmt {
    ListPendingInSchedulerOrderStmt(
        "SELECT * FROM builds WHERE status = 'pending' ORDER BY priority DESC, created_at ASC LIMIT $1 OFFSET $2",
        None,
    )
}
...
pub fn pending_feature_demand() -> PendingFeatureDemandStmt {
    PendingFeatureDemandStmt(
        "SELECT DISTINCT unnest(required_features) FROM builds WHERE status = 'pending' AND system = $1",
        None,
    )
}
...
pub fn reset_orphaned() -> ResetOrphanedStmt {
    ResetOrphanedStmt(
        "UPDATE builds SET status = 'pending', started_at = NULL WHERE status = 'running' AND started_at < NOW() - make_interval(secs => $1)",
        None,
    )
}
impl ResetOrphanedStmt {
    ...
    pub async fn bind<'c, 'a, 's, C: GenericClient>(
        &'s self,
        client: &'c C,
        older_than_secs: &'a f64,
    ) -> Result<u64, tokio_postgres::Error> {
        client.execute(self.0, &[older_than_secs]).await
    }
}
...
pub fn restart() -> RestartStmt {
    RestartStmt(
        "UPDATE builds SET status = 'pending', started_at = NULL, completed_at = NULL, log_path = NULL, build_output_path = NULL, error_message = NULL, started_notified_at = NULL, retry_count = retry_count + 1 WHERE id = $1 AND status IN ('failed', 'succeeded', 'cancelled', 'cached_failure') RETURNING *",
        None,
    )
}
impl RestartStmt {
