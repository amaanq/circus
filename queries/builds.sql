-- Build CRUD and scheduling queries. All full-row queries share the `BuildRow`
-- type so the repo layer needs a single `From<BuildRow> for models::Build`.

--: BuildRow(started_at?, completed_at?, log_path?, build_output_path?, error_message?, system?, notification_pending_since?, outputs?, constituents?, builder_id?, agent_machine_id?, fod_hash?, meta_description?, meta_license?, meta_homepage?, meta_maintainers?, started_notified_at?)
...
active_shares AS (
  ...
  JOIN builds b2 ON b2.evaluation_id = e2.id AND b2.status = 'pending'
...
SELECT DISTINCT unnest(required_features)
FROM builds
...
WHERE status = 'running'
  AND started_at < NOW() - make_interval(secs => :older_than_secs);
