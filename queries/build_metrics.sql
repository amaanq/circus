-- Build metrics: per-build performance numbers plus aggregate time-series
-- analytics over the builds table. The full-row upsert shares `BuildMetricRow`
-- so the repo layer needs a single `From<BuildMetricRow> for models::BuildMetric`.
-- Every column in build_metrics is NOT NULL, so the row type has no `?` fields.
--: BuildMetricRow()
